...
# Code from https://github.com/DanilZherebtsov/verstack/tree/master
...
def combine_single_valued_bins(y_binned):
    ...
    if len(neighbors) > 0:
        ...
        neighbors = np.array_split(neighbors, splits) # type: ignore
...
class ContinuousStratifiedKFold(StratifiedKFold):
    def __init__(self, n_splits: int = 5, *, shuffle: bool = False, random_state: int | RandomState | None = None) -> None:
